/* TableTrack — app shell, role + route switching, mount */
...
function Topbar({ role, setRole, snap }) {
  return (
    <div className="topbar">
      ...
      <div className="sep" />
      <span className="venue">{TT.VENUE.name}</span>
    ...
    <div className="sidebar">
      ...
      {NAV.map((n) => (
        <button key={n.id} className={"nav-item" + (page === n.id ? " on" : "")} onClick={() => setPage(n.id)}>
          ...
          {n.id === "monitor" && <span className="badge">{c.available} open</span>}
        </button>
      ))}
      ...
      <div className="workcard">
        <div className="wc-cam">{TT.VENUE.camera} · {TT.VENUE.resolution.split(" · ")[0]}</div>
        <div className="wc-name">{TT.VENUE.area}</div>
        <div className="wc-meta">
          <span className="dot" style={{ background: snap.running ? "var(--ok)" : "var(--muted)" }} />
          {snap.running ? "Detection running" : "Detection paused"} · {TT.TABLES.length} zones
        </div>
      </div>
...
function App() {
  ...
  function go(target) {
    if (target === "customer" || target === "public" || target === "admin") { setRole(target); return; }
