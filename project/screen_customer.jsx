/* TableTrack — Customer Seating Map + Public Display (TV) */

function GroupFilter({ value, onChange, dark }) {
  const opts = [{ v: 0, l: "Any size" }, { v: 1, l: "1" }, { v: 2, l: "2" }, { v: 4, l: "3–4" }, { v: 6, l: "5+" }];
  return (
    <div className="seg-filter" style={dark ? { } : {}}>
      {opts.map((o) => (
...
function CustomerMap() {
  ...
  const openTables = TT.TABLES
    .filter((t) => snap.tables[t.id].status === "available" && (group === 0 || t.capacity >= group))
      ...
      <div className="cust-wrap fadein">
        ...
        <div style={{ display: "flex", alignItems: "flex-end", gap: 16, flexWrap: "wrap", marginBottom: 20 }}>
          <div>
            ...
            <h1 style={{ fontSize: 34, fontWeight: 780, letterSpacing: "-.03em", margin: "4px 0 0" }}>{TT.VENUE.area}</h1>
          </div>
          <div style={{ flex: 1 }} />
          <div className={"live-chip" + (snap.running ? "" : " paused")} style={{ fontSize: 13 }}>
            <span className="live-dot" />{snap.running ? "Live" : "Paused"} · updated {TT.timeAgo(snap.updatedAt)}
          </div>
        </div>
        ...
        <div className="card card-pad" style={{ display: "flex", alignItems: "center", gap: 26, padding: "22px 24px", marginBottom: 18 }}>
          <div>
            <div style={{ fontSize: 52, fontWeight: 800, letterSpacing: "-.04em", lineHeight: 1, color: c.available + c.partial > 0 ? "var(--ok-ink)" : "var(--busy-ink)" }}>
        ...
        <div className="card" style={{ padding: 14, overflow: "hidden" }}>
          <div style={{ display: "flex", alignItems: "center", padding: "6px 8px 12px", gap: 14 }}>
            ...
            <div className="hint" style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <Icon name="dot" size={14} /> hover a table for details · north = window wall
            </div>
          </div>
        ...
        <div style={{ marginTop: 18 }}>
          ...
          {openTables.length === 0 ? (
            <div className="card card-pad hint" style={{ color: "var(--busy-ink)" }}>
              No fully-open tables match right now — partially-occupied tables may still have room. Check the map above.
            </div>
          ) : (
              ...
              {openTables.slice(0, 8).map((t) => (
                <div key={t.id} className="card" style={{ padding: "11px 14px", display: "flex", alignItems: "center", gap: 11 }}>
                  ...
                  <div>
                    ...
                    <div className="mono" style={{ fontSize: 11, color: "var(--ink-3)" }}>seats {t.capacity} · {labelType(t.type)}</div>
                  </div>
                </div>
              ))}
            </div>
...
function labelType(t) {
  return { round: "round table", window: "window seat", booth: "booth", communal: "communal", lounge: "lounge" }[t] || t;
}
    ...
    <div className="public" style={{ padding: "clamp(20px,3vw,44px)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: "clamp(14px,2vw,26px)" }}>
        ...
        <div style={{ minWidth: 0 }}>
          <div style={{ fontWeight: 800, fontSize: "clamp(18px,1.6vw,26px)", letterSpacing: "-.02em", whiteSpace: "nowrap" }}>{TT.VENUE.name}</div>
          <div style={{ color: "#b7ad9a", fontSize: "clamp(12px,1vw,15px)", fontWeight: 600, whiteSpace: "nowrap" }}>{TT.VENUE.area} · live seating</div>
        </div>
        <div style={{ flex: 1 }} />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(280px,1fr) 1.55fr", gap: "clamp(18px,2.5vw,40px)", flex: 1, minHeight: 0, alignItems: "center" }}>
        {/* big readout */}
        <div>
          <div style={{ fontSize: "clamp(13px,1.1vw,17px)", color: "#b7ad9a", fontWeight: 700, letterSpacing: ".14em", textTransform: "uppercase" }}>Tables open now</div>
...
Object.assign(window, { CustomerMap, PublicDisplay, GroupFilter, Legend, Stat, labelType });
